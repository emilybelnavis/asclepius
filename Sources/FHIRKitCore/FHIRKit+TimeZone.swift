...
public extension TimeZone {
    ...
    static func hs_parseComponents(from scanner: Scanner, expectAtEnd: Bool = true) throws -> (secondsFromGMT: Int, timeZoneString: String) {
        ...
        guard let tzPrefix = scanner.hs_scanCharacters(from: plusMinusZ) else {
            throw FHIRKitDateParserError.invalidTimeZonePrefix(FHIRKitDateParserErrorPosition(string: scanner.string, location: scanLocation))
        }
        ...
        if "Z" == tzPrefix {
            ...
            guard hourString.count == 2 else {
                throw FHIRKitDateParserError.invalidSeparator(FHIRKitDateParserErrorPosition(string: scanner.string, location: scanLocation + min(2, hourString.count)))
            }
            ...
            guard let minuteString = scanner.hs_scanCharacters(from: numbers) else {
                throw FHIRKitDateParserError.invalidTimeZoneMinute(FHIRKitDateParserErrorPosition(string: scanner.string, location: scanLocation))
            }
            guard minuteString.count <= 2 else {
                throw FHIRKitDateParserError.additionalCharacters(FHIRKitDateParserErrorPosition(string: scanner.string, location: scanLocation + min(2, minuteString.count)))
            }
